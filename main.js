...
const CA = '0x27c5260a22DF0e4FD2DAAA8085cfA9a1c67Cb1Ed';
const ABI = [
  {
    "inputs": [
      {
...
document.getElementById('nft-gen-btn').addEventListener('click', async () => {
  ...
  const rst = await ipfs.add(file);

  const account = web3.eth.accounts.privateKeyToAccount(PK);
  const name = document.getElementById('nft-name').value;

  const bytedata = await contract.methods.mint(account.address, name, rst.path).encodeABI();
...
document.getElementById('nft-search-btn').addEventListener('click', async () => {
  ...
  const { name, url } = await contract.methods.images(tokenId).call();

  document.getElementById('nft-owner-address-search').innerText = address;
  document.getElementById('nft-name-search').innerText = name;
  document.getElementById('ipfs-path-search').innerHTML = `<a href="http://localhost:5001/webui/#/explore/${url}" target="_blank">${url}</a>`;
})